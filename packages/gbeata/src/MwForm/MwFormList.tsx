...
import { MwFormField, MwFormProps } from './mw-form'
...
export default function MwFormList(props: AyFormListProps) {
          ...
          <>
            {fields.map(({ key, name, ...restField }) => {
              ...
              let content = getFormItem(
                children.map((child: MwFormField) => {
                  let newField: MwFormField = {
                    ...child,
                    formItemProps: {
                      ...field.formItemProps,
                      ...restField,
                  ...
                  if (readonly && !newField.render) {
                    try {
                      ...
                      console.error('mw-form-list 转换错误')
                    }
                  }
                  return newField
                }) as Array<MwFormField | MwSearchTableField>,
                formInstant,
                ayFormProps,
                FORM_TYPE_LIST
              )
              return (
                <Space key={`${field.key}-${key}`} className="mw-form-list-item" align="end" {...field.spaceProps}>
                  {content}
                  {!readonly && (
                    <Space className="mw-form-list-actions">
                      {recordNum < max && (
                        <span className="mw-form-list-action" onClick={() => handleCopy(name)}>
                      ...
                      {recordNum > min && (
                        <span className="mw-form-list-action" onClick={() => handleRemove(name, remove)}>
