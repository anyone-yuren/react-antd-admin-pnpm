...
const getSearchFields = (
  ...
  /** 是否平铺展示 */
  inline: boolean
): { searchFields: Array<MwFormField>; span: number } => {
  // 累计的 span
  let spanSum: number = 0
  // 每一行占用的 span，超过 24 清 0
  let rowSpanSum: number = 0
  let newFields: Array<MwSearchField | MwFormField> = fields
    // 过滤掉隐藏的
  ...
  newFields = newFields.map((field, index) => {
    ...
    // MwForm 的 span 值是具体数值
    newField.span = newSpan
    return newField
  })

  return {
    searchFields: newFields as Array<MwFormField>,
    span: spanSum
  }
}
...
export default forwardRef(function MwSearch(props: MwSearchProps, ref) {
  ...
  return (
    <Card className={classNames('mw-search', inline ? '' : 'full-width', inline ? 'inline' : '')}>
        ...
        >
          {actionVisible !== false && (
            <Col span={actionSpan} style={actionStyle}>
              <Form.Item>
                <Space>
                  <MwButton htmlType="submit" type="primary" icon={<SearchOutlined />}>
