...
import { MwFormField, MwFormProps, RegisterFieldProps } from './mw-form'
 ...
 * @param field 配置项
 */
const getNoVisibleField = (field: MwFormField | MwSearchTableField): MwFormField | MwSearchTableField => {
  return {
    ...field,
    title: '',
    type: 'empty'
 ...
 * @param fields 配置列表
 */
export const getDefaultValue = (fields: Array<MwFormField | MwSearchField | MwSearchTableField>) => {
...
export const getFieldDefaultValue = (key: string, fields: Array<MwFormField | MwSearchField | MwSearchTableField>) => {
  if (!key) {
    return ''
  }
...
const getFormItem = (
  ...
  const ayFormProps: MwFormProps = props

  return fields.map((field: MwFormField | MwSearchTableField, index: number) => {
    // 把其它属性 添加到 props 里面
    field = {
      ...field,
      props: {
...
const getField = (key: string, fields: Array<MwFormField | MwSearchTableField>) => {
 ...
 * @param fields 配置项
 */
const formatValues = (values: AnyKeyProps, fields: Array<MwFormField | MwSearchTableField>): AnyKeyProps => {
...
export default forwardRef(function MwForm(props: MwFormProps, ref: Ref<any>) {
  const {
    fields: originFields,
   ...
   * @returns object
   */
  const getValues = (fields: MwFormField | MwSearchTableField, readonly?: boolean) => {
    let result: AnyKeyProps = {}
