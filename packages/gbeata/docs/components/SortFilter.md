...
gbeata 精简了原来的 antd 提供的配置，`sort: true` 指定此列需要排序，`filter: true` 指定此列需要筛选。
...
/**
 * title: 排序
 * desc: gbeata 默认就是服务端排序和筛选；多列排序需要额外设置 `sortOrder`，数字越大权重越高；多选筛选需要指定 `filterMultiple`。
 */
...
export default function Demo() {
  ...
  return (
    <GSearchTable
      ...
      fields={fields}
    >
      <GButton onClick={() => ref.current.clearFilters()}>
        清空全部过滤值
      </GButton>
      <GButton onClick={() => ref.current.clearSorts()}>清空全部排序</GButton>
      <GButton
        onClick={() => {
...
export default function Demo() {
  ...
  return (
    ...
    >
      <GButton
        onClick={() =>
          ref.current.setSortsValue([{ key: 'ori-atk', order: 'ascend' }])
        }
      >
        设置初始攻击升序
      </GButton>
      <GButton onClick={() => ref.current.setFiltersValue({ rarity: ['2'] })}>
        设置⭐️⭐️⭐️筛选
      </GButton>
      <GButton
        onClick={() => {
      ...
      <GButton
        onClick={() => alert(JSON.stringify(ref.current.getApiParams()))}
      >
        获取请求前参数
      </GButton>
    </GSearchTable>
